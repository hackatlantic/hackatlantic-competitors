...
| `Production` | `HCP_TERRAFORM_TOKEN`, `TFVARS_PRODUCTION_JSON`, `API_OBSERVABILITY_ENV_JSON`, `DIGITALOCEAN_ACCESS_TOKEN`, `SPACES_ACCESS_KEY_ID`, `SPACES_SECRET_ACCESS_KEY`, `SUPABASE_ACCESS_TOKEN`, `VERCEL_TOKEN`, `VERCEL_ORG_ID`, `VERCEL_PROJECT_ID` | Production URLs; the existing environment has required reviewers (names are case-insensitive) |
...
For the native OTLP gateway, Basic authentication uses the **OTLP instance ID**
from the stack's OpenTelemetry connection details, not its Prometheus tenant ID.
Validate the header using an empty OTLP request before storing it; never print
the header or decoded token. A successful API deployment does not prove that
telemetry export is authenticated.
