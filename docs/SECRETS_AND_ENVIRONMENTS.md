...
Repository-level secrets used by unprivileged pull-request checks should be avoided. Dependabot and forked pull requests never receive protected environment secrets. HCP Terraform sensitive variables hold provider/runtime configuration; GitHub stores only the token needed to execute the workspace.
